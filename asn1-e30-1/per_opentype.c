...
uper_open_type_get_simple(asn_codec_ctx_t *ctx, asn_TYPE_descriptor_t *td,
	...
	buftmp = pd->buffer;
	tmplen = ((unsigned int)(pd->nbits - pd->nboff)) >> 3;
	printf("[ZGB DEBUG]: pd = ");
	for(i = 0;i < tmplen; i++)
	{
		printf("0x%x ",*buftmp);
...
//	printf("Getting open type %s...\r\n", td->name);
	...
	printf("Getting open type %s encoded in %ld bytes\r\n", td->name,
		(long)bufLen);
	...
	if(rv.code == RC_OK) {
		...
		if(padding >= 8) {
			...
			printf("Too large padding %d in open type\r\n", (int)padding);
			padding = padding % 8;
			//_ASN_DECODE_FAILED;
		} else {
