...
SEQUENCE_decode_uper(asn_codec_ctx_t *opt_codec_ctx, asn_TYPE_descriptor_t *td,
	...
	//printf("[ZGB DEBUG]: (%s:%d)\n",__FILE__,__LINE__);
	printf("[ZGB DEBUG]: Decoding %s as SEQUENCE (UPER) \n", td->name);
	...
	for(edx = 0; edx < td->elements_count; edx++) {
		...
		if(elm->optional) {
			...
			printf("[ZGB DEBUG]: Member %s->%s is optional, p=%d (%d->%d) \n", td->name, elm->name, present,(int)opmd.nboff, (int)opmd.nbits);
		...
		printf("[ZGB DEBUG]: Decoding member %s in %s \n", elm->name, td->name);
		ASN_DEBUG("Decoding member %s in %s", elm->name, td->name);
		rv = elm->type->uper_decoder(opt_codec_ctx, elm->type,
			elm->per_constraints, memb_ptr2, pd);
		if(rv.code != RC_OK) {
			printf("[ZGB DEBUG]: Failed decode %s in %s \n",elm->name, td->name);
	...
	if(extpresent) {
		...
		printf("[ZGB DEBUG]: Extensions %ld present in %s \n",(long)bmlength, td->name);
		...
		epmd.buffer = epres;
		epmd.nbits = bmlength;
		printf("[ZGB DEBUG]: Read in extensions bitmap for %s of %ld bits (%x..) \n",td->name, (long)bmlength, *epres);
		...
		printf("[ZGB DEBUG]: extensions %s \n",elm->name);
		if(!IN_EXTENSION_GROUP(specs, edx)) {
			printf("[ZGB DEBUG]: %d is not extension \n", edx);
		...
		if(present <= 0) {
			printf("[ZGB DEBUG]: No more extensions \n");
			if(present < 0) break;	/* No more extensions */
			continue;
		}

		printf("[ZGB DEBUG]: Decoding member %s in %s %p \n", elm->name, td->name, *memb_ptr2);
		...
		for(;;) {
			printf("[ZGB DEBUG]: Getting overflow extensions \n");
